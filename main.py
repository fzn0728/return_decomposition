...
def rolling_coef(df):
    ...
    m_df = pd.DataFrame(columns=['ACWI', 'MSCI_World', 'Russell_3000', 'US_Dollar', 'Bond_Index', 'HFRI_World_Index', 'HFRI_Relative_Value', 'HFRI_Macro', 'HFRI_Macro_Total', 'HFRI_ED', 'HFRI_EH'])
    for i in range(0,len(df.index)-35):
        df_r = df.iloc[i:i+36,:]
        my_ols = sm.ols(formula='Kroger ~ ACWI + MSCI_World + Russell_3000 + US_Dollar + Bond_Index + HFRI_World_Index + HFRI_Relative_Value + HFRI_Macro + HFRI_Macro_Total + HFRI_ED + HFRI_EH', data=df_r).fit()
        # concat each rolling param
        s = pd.DataFrame(my_ols.params).T
        m = pd.DataFrame(df_r.mean(axis=0)).T
...
def rolling_orth_coef(df):
    ...
    for i in range(0,len(df.index)-35):
        ...
        my_ols = sm.ols(formula='Kroger ~ ACWI_err + MSCI_World_err + Russell_3000_err + US_Dollar + Bond_Index + HFRI_World_Index_err + HFRI_Relative_Value_err + HFRI_Macro_err + HFRI_Macro_Total_err + HFRI_ED_err + HFRI_EH', data=df_r).fit()
        # concat each rolling param
        s = pd.DataFrame(my_ols.params).T
        para_df = pd.concat([para_df,s], axis=0)
    # chean format of index
...
def rolling_v_coef(df):
    ...
    m_df = pd.DataFrame(columns=['ACWI_err', 'MSCI_World_err', 'Russell_3000', 'US_Dollar', 'Bond_Index','HFRI_World_Index','HFRI_Relative_Value', 'HFRI_Macro_Total','HFRI_ED','HFRI_EH'])
    for i in range(0,len(df.index)-35):
        df_r = df.iloc[i:i+36,:]
        # gen orthogonal error series
        df_r['ACWI_err'] = get_error(df_r, 'ACWI ~ MSCI_World + Russell_3000 + US_Dollar + Bond_Index + HFRI_World_Index + HFRI_Relative_Value + HFRI_Macro + HFRI_Macro_Total + HFRI_ED + HFRI_EH').values
        ...
        my_ols = sm.ols(formula='Kroger ~ ACWI_err + MSCI_World_err + Russell_3000 + US_Dollar + Bond_Index + HFRI_World_Index + HFRI_Relative_Value + HFRI_Macro_Total + HFRI_ED + HFRI_EH', data=df_r).fit()
        # concat each rolling param
        s = pd.DataFrame(my_ols.params).T
        m = pd.DataFrame(df_r.loc[:,['ACWI_err', 'MSCI_World_err', 'Russell_3000', 'US_Dollar', 'Bond_Index','HFRI_World_Index','HFRI_Relative_Value', 'HFRI_Macro_Total','HFRI_ED','HFRI_EH', 'Kroger']].mean(axis=0)).T
        para_df = pd.concat([para_df,s], axis=0)
        m_df = pd.concat([m_df,m], axis=0)
    # chean format of index
    para_df = para_df.reset_index()
...
if __name__ == '__main__':
    ...
    mean_df, para_df = rolling_coef(df)
    # para_df.plot()
    # plt.title('Evolution of Coefficient - Classical Method')
    # Calculate Prediction
    pred_df = plot_pre(para_df, mean_df, 'Prediction - Classical Method', ['ACWI', 'MSCI_World', 'Russell_3000', 'US_Dollar', 'Bond_Index', 'HFRI_World_Index', 'HFRI_Relative_Value', 'HFRI_Macro', 'HFRI_Macro_Total', 'HFRI_ED', 'HFRI_EH'])
    # Revise it into proportion
    para_por_df = para_df.loc[:,['ACWI', 'MSCI_World', 'Russell_3000', 'US_Dollar', 'Bond_Index', 'HFRI_World_Index', 'HFRI_Relative_Value', 'HFRI_Macro', 'HFRI_Macro_Total', 'HFRI_ED', 'HFRI_EH']]
    para_por_df = para_por_df.apply(lambda x: x/x.sum(), axis=1)
    para_por_abs_df = para_df.loc[:,['ACWI', 'MSCI_World', 'Russell_3000', 'US_Dollar', 'Bond_Index', 'HFRI_World_Index', 'HFRI_Relative_Value', 'HFRI_Macro', 'HFRI_Macro_Total', 'HFRI_ED', 'HFRI_EH']].abs()
    ...
    # Revise it into proportion
    para_por_orth_df = para_orth_df.loc[:,['ACWI_err', 'MSCI_World_err', 'Russell_3000_err', 'US_Dollar', 'Bond_Index','HFRI_World_Index_err','HFRI_Relative_Value_err','HFRI_Macro_err','HFRI_Macro_Total_err','HFRI_ED_err','HFRI_EH']]
    para_por_orth_df = para_por_orth_df.apply(lambda x: x/x.sum(), axis=1)
    para_por_orth_abs_df = para_orth_df.loc[:,['ACWI_err', 'MSCI_World_err', 'Russell_3000_err', 'US_Dollar', 'Bond_Index','HFRI_World_Index_err','HFRI_Relative_Value_err','HFRI_Macro_err','HFRI_Macro_Total_err','HFRI_ED_err','HFRI_EH']].abs()
    ...
    # Calculate Prediction
    pred_v_df = plot_pre(para_v_df, mean_v_df, 'Prediction - VIF Adjusted Method', ['ACWI_err', 'MSCI_World_err', 'Russell_3000', 'US_Dollar', 'Bond_Index', 'HFRI_World_Index', 'HFRI_Relative_Value', 'HFRI_Macro_Total', 'HFRI_ED', 'HFRI_EH'])
    # Revise it into proportion
    para_por_v_df = para_v_df.loc[:,['ACWI_err', 'MSCI_World_err', 'Russell_3000', 'US_Dollar', 'Bond_Index','HFRI_World_Index','HFRI_Relative_Value', 'HFRI_Macro_Total','HFRI_ED','HFRI_EH']]
    para_por_v_df = para_por_v_df.apply(lambda x: x/x.sum(), axis=1)    
    para_por_v_abs_df = para_v_df.loc[:,['ACWI_err', 'MSCI_World_err', 'Russell_3000', 'US_Dollar', 'Bond_Index','HFRI_World_Index','HFRI_Relative_Value', 'HFRI_Macro_Total','HFRI_ED','HFRI_EH']].abs()
